...
mutable struct OptimizationState{Interp<:AbstractInterpreter}
    ...
    bb_vartables::Vector{Union{Nothing,VarTable}} # the varstate of each slot at each block entrypoint
end
...
function slot2reg(ir::IRCode, ci::CodeInfo, sv::OptimizationState)
    ...
    # TODO: this is bad!!
    if length(sv.bb_vartables) != length(ir.cfg.blocks)
        filter!(bb_state -> bb_state !== nothing, sv.bb_vartables)
    end
    # @assert length(sv.bb_vartables) == length(ir.cfg.blocks)
